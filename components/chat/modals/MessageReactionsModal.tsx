...
export const MessageReactionsModal: React.FC<Props> = ({
  ...
  return (
    <Modal visible transparent animationType="none" presentationStyle="overFullScreen" statusBarTranslucent onRequestClose={onClose}>
      <View style={{ flex: 1 }}>
        <Pressable
        ...
        >
          <View className="rounded-t-[28px] bg-white px-4 pb-6 pt-3 ">
              ...
              >
                <View className="flex-row gap-2">
                  <Pressable
                    ...
                    className={`flex-row items-center rounded-full border px-3 py-2 ${!selectedReactionType ? 'border-[#b78457] bg-[#fff8f1]' : 'border-slate-200 bg-white'}`}
                  >
                    <Feather name="layers" size={13} color={!selectedReactionType ? '#b78457' : '#64748b'} />
                    <Text className={`ml-2 text-[13px] font-semibold ${!selectedReactionType ? 'text-[#8b5e34]' : 'text-slate-600'}`}>
                      Tất cả
                    </Text>
                  </Pressable>
                  ...
                  {reactionTypeOptions.map((item) => {
                    ...
                    return (
                      <Pressable
                        ...
                        className={`flex-row items-center rounded-full border px-3 py-2 ${isSelected ? 'border-[#b78457] bg-[#fff8f1]' : 'border-slate-200 bg-white'}`}
                      >
                        <Text className="text-[15px]">{item.emoji}</Text>
                        <Text className={`ml-2 text-[13px] font-semibold ${isSelected ? 'text-[#8b5e34]' : 'text-slate-600'}`}>
                          {item.count}
                        </Text>
                      </Pressable>
                    );
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 360 }} contentContainerStyle={{ paddingBottom: 10 }}>
              ...
              {filteredReactionItems.length === 0 && (
                <View className="items-center py-10">
                  ...
                  <Text className="mt-2 text-[13px] text-slate-500">
                    {selectedReactionType ? 'Chưa có ai thả loại cảm xúc này' : 'Chưa có ai thả cảm xúc'}
                  </Text>
                </View>
              )}
            </ScrollView>
